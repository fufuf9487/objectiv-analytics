...
class ExtractedContextsPipeline(BaseDataPipeline):
    ...
    def validate_pipeline_result(cls, result: bach.DataFrame) -> None:
        check_objectiv_series_dtypes(
            result, columns_to_check=list(ObjectivSupportedColumns.get_extracted_context_columns()),
        )
...
class BigQueryExtractedContextsPipeline(ExtractedContextsPipeline):
    ...
    BQ_TAXONOMY_COLUMN = 'contexts_io_objectiv_taxonomy_1_0_0'
        ...
        BQ_TAXONOMY_COLUMN: [
            {
                ...
                '_types': [bach.SeriesString.dtype],
                'cookie_id': bach.SeriesUuid.dtype,
                'event_id': bach.SeriesUuid.dtype,
                'global_contexts': [bach.SeriesDict.dtype],
                'location_stack': [bach.SeriesDict.dtype],
    ...
    def _process_base_data(self, context_df: bach.DataFrame) -> bach.DataFrame:
        ...
        taxonomy_columns_dtypes = context_dtypes[self.BQ_TAXONOMY_COLUMN][0]
        other_columns_dtypes = {
            col: dtype for col, dtype in context_dtypes.items() if col != self.BQ_TAXONOMY_COLUMN
        }
        ...
        sql = (
            ...
            f'from {{{{context_node}}}} '
            f'cross join unnest({quote_identifier(dialect, self.BQ_TAXONOMY_COLUMN)}) '
...
def get_extracted_contexts_df(engine: Engine, table_name: str, **kwargs) -> bach.DataFrame:
